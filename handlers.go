...
func LogError(message string) {
    // f, err := os.OpenFile("errlog.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
    // Using an absolute path is safer if you're launching from different directories
    f, err := os.OpenFile("./errlog.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
    if err != nil {
        return // If we can't log, we're really in trouble
    }
    defer f.Close()
...
func handleOpenNewWindow(call JSCall) string {
    if len(call.Args) < 5 { 
        LogError("OpenNewWindow: Not enough arguments")
        return "false" 
    }
    ...
    // 1. AUTO-RESOLVE RELATIVE PATHS
    if addrType == "HTMLAddress" {
        // If the user didn't provide a protocol (like http:// or file://)
        // we assume it's a relative file in the current working directory.
        if !strings.HasPrefix(source, "file://") && !strings.HasPrefix(source, "http") {
            // filepath.Join handles the slashes correctly for the OS
            absPath := filepath.Join(pwd, source)
            finalSource = "file://" + filepath.ToSlash(absPath)
            LogError(fmt.Sprintf("Resolved relative path [%s] to [%s]", source, finalSource))
        }
    }

    // 2. Perform decoding logic
    if addrType == "HTMLString" {
        decoded, err := base64.StdEncoding.DecodeString(source)
        if err != nil {
            LogError(fmt.Sprintf("Base64 Decode Error for window %s: %v", name, err))
            return "false"
        }
        finalSource = string(decoded)
        //LogError(fmt.Sprintf("HTMLString decoded for %s. Length: %d chars", name, len(finalSource)))
    }
    ...
    if ptr == nil { 
        LogError("C Bridge returned a NULL pointer for window: " + name)
        return "false" 
    }
